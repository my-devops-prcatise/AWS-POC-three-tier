...
resource "aws_route_table" "private_rt_1" {
  vpc_id = aws_vpc.main.id
  tags = { Name = "Private-Route-Table-DB" }
}

# No aws_route for 0.0.0.0/0 here (keep DB isolated)
...
# --- Private App Route Table (needs outbound via NAT) ---
resource "aws_route_table" "private_rt_2" {
  vpc_id = aws_vpc.main.id
  tags = { Name = "Private-Route-Table-APP" }
}

resource "aws_route" "private_app_nat_access" {  # renamed to avoid duplicate
